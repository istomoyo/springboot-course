...
import com.example.experiment7.compoent.ULID;
import com.example.experiment7.dto.Item;
import com.example.experiment7.dto.Order;
...
public class OrderService {
    ...
    // 传入抢购的商品，抢购用户
    public long buy(Item item, String userId) {
        var key = Item.PRE_KEY + item.getId();
        // 调用redis抢购函数
        long quantity = client
                .getFunction()
                .call(FunctionMode.WRITE, "buy_13", FunctionResult.LONG, List.of(key));
        if (quantity == -1) {
            log.debug("抢光啦");
            return quantity;
        }
        ...
        RBucket<Order> bucket = client.getBucket(Order.PRE_KEY + order.getId());
        // 持久化订单
        bucket.set(order);
